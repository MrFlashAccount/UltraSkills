...
- [`ui-design-proposal-template.html`](ui-design-proposal-template.html): first-class inspectable UI design proposal that embeds sibling raster image artifacts for 3-4 materially comparable directions when needed, selected desktop/mobile views, and important screens/states; HTML carries product/surface routing, design-basis preflight, user decisions, annotations, hierarchy and taste contracts, evidence expectations, and open tensions rather than drawing the proposed UI itself. Card/list and drawer/sidebar/bottom-sheet sections are conditional selected-pattern contracts, not universal composition defaults.
