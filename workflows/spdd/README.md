...
Architecture generation is direct by default. An explicit dialectic request inserts one isolated Visionary/Pragmatist position fanout and one independent synthesis fanout before `architecture_draft`; omission or ambiguity remains direct. Optional UI work still reaches the selected architecture-generation step only after the existing UI approval gate. Convergence or one adjudication selects a traceable lineage, then the same stable `spdd_architecture_owner` materializes the sole Canvas. Worker restoration is advisory: position and synthesis phases must remain correct with a fresh worker from typed artifacts alone. The existing `architecture_attack`, architecture approval, implementation fanout, review fanout, and final approval remain mandatory and unchanged after any canonical dialectic result.

Reject-both or irreparable invalidity routes to `architecture_dialectic_exit`, preserves evidence, and terminates before Canvas creation, architecture approval, implementation, or review. A non-blocking stop is reserved for one concrete answer that can resume adjudication; a legitimate no-synthesis result is terminal evidence, not a pause.
