...
import { callFunction, continueRun, listPointerTransitions, loadInstructions, movePointer, next, reportStop, resolveStop, writeOutput } from '../workflow-runner-command.mjs';
...
function parseCliArgs(argv) {
  ...
  if (!['next', 'continue', 'instructions', 'write-output', 'call-function', 'report-stop', 'resolve-stop', 'list-pointer-transitions', 'move-pointer'].includes(mode)) fail(usage());
  try {
    ...
    if (['instructions', 'write-output', 'call-function', 'report-stop', 'resolve-stop'].includes(mode) && !parsed.values['step-id']) fail(usage());
    if (!['instructions', 'write-output', 'call-function', 'report-stop', 'resolve-stop'].includes(mode) && parsed.values['step-id']) fail(usage());
