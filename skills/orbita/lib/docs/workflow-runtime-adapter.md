...
- `wait_for_approval` does not spawn a worker. The wrapper/orchestrator follows the compact inline approval prompt in runner stdout, reads only files under `Required reads`, renders files under `Approval attachments` without opening them, shows the supplied summary/review context, asks only for the requested decision/input, and writes the normalized approval JSON with the validating writer command from that prompt.
