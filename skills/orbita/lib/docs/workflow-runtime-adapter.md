...
Harness matching is ASCII case-insensitive: new successful claims persist a lowercase harness, while runtime also folds legacy mixed-case claim state and workflow keys. Workflow keys that differ only by case are rejected as ambiguous. On a match, the executable source worker's `run_worker` request includes `agentRuntime: { model, thinkingLevel }`, and `--only-instructions` adds one short sentence recommending that model and thinking level for a fresh subagent. The host uses the recommendation when available and falls back to its default runtime when the model or thinking level is unavailable, without blocking the workflow. Orbita leaves configured model and thinking values unchanged. A missing source agent, claim harness, or profile match emits neither the field nor the sentence. Restored preferred workers keep their existing runtime; a fresh worker applies the preference or host fallback.
