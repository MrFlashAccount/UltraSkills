...
export class RunsRootObserverReader {
  ...
  constructor(readonly runsRoot: string, private readonly now: () => Date = () => new Date()) {}
  ...
  private workflowDocument(workflowPath: unknown): unknown {
    ...
    if (!this.workflowCache.has(workflowPath)) this.workflowCache.set(workflowPath, readWorkflowDocument(workflowPath, 'workflow'));
    return this.workflowCache.get(workflowPath);
  }

  private async loadEntry(entry: any, signal?: AbortSignal): Promise<{ run: any; persistedState?: any; workflowDocument?: any; degraded?: boolean }> {
    try {
      signal?.throwIfAborted();
      const lookupPaths = resolveRunPaths({ runId: entry.runId, workflowPath: entry.workflow?.path, runsRoot: this.runsRoot });
      ...
      const paths = resolveRunPaths({ runId: run.runId, workflowPath: run.workflow?.path, runsRoot: this.runsRoot });
  ...
  async listRuns(signal?: AbortSignal): Promise<RunSummaryDTO[]> {
    ...
    return mapWithConcurrency(entries, RUN_READ_CONCURRENCY, async (entry) => projectRunSummary(await this.loadEntry(entry, signal), { now: this.now() }));
  }
