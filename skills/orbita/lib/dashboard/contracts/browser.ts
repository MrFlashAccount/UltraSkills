...
const PublicTextSourceSchema = z.enum(Object.keys(PUBLIC_TEXT_LIMITS) as [PublicTextSource, ...PublicTextSource[]]);

function utf8Length(value: string): number {
  return new TextEncoder().encode(value).byteLength;
}

export const PublicDisplayTextSchema = z.object({
  ...
  if (Array.from(text.value).length > limits.codePoints) {
    context.addIssue({ code: 'custom', message: 'public text exceeds its code-point ceiling', path: ['value'] });
  }
  if (utf8Length(text.value) > limits.utf8Bytes) {
    context.addIssue({ code: 'custom', message: 'public text exceeds its UTF-8 byte ceiling', path: ['value'] });
  }
});
export type PublicDisplayText = z.infer<typeof PublicDisplayTextSchema>;

const IsoDateSchema = z.string().datetime({ offset: true });
export const RunIdSchema = z.string().min(1).max(160).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/u);
export const WorkflowIdentitySchema = z.string().min(1).max(120).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/u);
export const StepIdSchema = z.string().min(1).max(160).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/u);
const ArtifactIdSchema = z.string().min(1).max(160).regex(/^[A-Za-z0-9][A-Za-z0-9._:/-]*$/u);
const ResultRefSchema = z.string().min(1).max(160).regex(/^[A-Za-z0-9][A-Za-z0-9._:/-]*$/u);
...
export const ObserverFreshnessSchema = z.object({
...
export const RunSummarySchema = z.object({
  ...
  status: z.string().min(1).max(48).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/u).optional(),
...
const DetailFactSchema = z.object({
  label: z.enum(['Run id', 'Workflow', 'Current step']),
  value: z.string().min(1).max(160).regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/u),
}).strict();

const DetailArtifactSchema = z.object({
  producerStepId: StepIdSchema.optional(),
  id: ArtifactIdSchema,
  contentType: z.string().min(1).max(120).regex(/^[\w.+-]+\/[\w.+-]+$/u).optional(),
...
export const RunDetailSchema = RunSummarySchema.extend({
  ...
  const totalHistoryBytes = detail.history.reduce((total, line) => total + utf8Length(line.value), 0);
  if (totalHistoryBytes > 8_192) {
    context.addIssue({ code: 'custom', message: 'history exceeds its total UTF-8 byte ceiling', path: ['history'] });
  }
});
...
export const PublicErrorSchema = z.object({
  error: z.object({
    code: z.enum(['not_found', 'method_not_allowed', 'observer_unavailable', 'invalid_request']),
