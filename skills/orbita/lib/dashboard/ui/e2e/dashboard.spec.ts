...
async function installStableEvents(page: Page) {
  await page.addInitScript(() => {
    ...
    window.fetch = async (...args) => {
      ...
      const counts = (
        window as unknown as { __orbitaFetchCounts: { snapshot: number; detail: number } }
      ).__orbitaFetchCounts;
...
async function emitInvalidations(
  ...
  await page.evaluate(
    async ({ firstChangeId, count, reason }) => {
      const source = (window as unknown as { __orbitaEventSources: EventTarget[] })
...
test('responsive attention board, detail semantics, virtualization, and keyboard proof', async ({
  ...
  expect(interactiveSamples[Math.ceil(interactiveSamples.length * 0.95) - 1]).toBeLessThan(2_000);
  ...
  await page.evaluate(() => {
    (window as unknown as { __orbitaLongTasks: number[] }).__orbitaLongTasks.length = 0;
  });
  await page.evaluate(() => {
    const counts = (
      window as unknown as { __orbitaFetchCounts: { snapshot: number; detail: number } }
  ...
  const longestTask = await page.evaluate(() =>
    Math.max(0, ...(window as unknown as { __orbitaLongTasks: number[] }).__orbitaLongTasks),
  );
