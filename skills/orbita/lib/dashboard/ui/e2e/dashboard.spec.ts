...
async function mockDashboard(
  ...
  const snapshot = buildSnapshot(1000, distribution);
...
async function installStableEvents(page: Page) {
  await page.addInitScript(() => {
    const eventSources: Array<EventTarget> = [];
    ...
    Object.defineProperty(window, "__orbitaLongTasks", { value: [] as Array<number> });
    Object.defineProperty(window, "__orbitaSnapshotResponseAt", { value: 0, writable: true });
    Object.defineProperty(window, "__orbitaFetchCounts", { value: { detail: 0, snapshot: 0 } });
    ...
    Object.defineProperty(window, "__orbitaReconciliationLatency", {
    ...
    window.fetch = async (...args) => {
      ...
      const counts = (
        window as unknown as { __orbitaFetchCounts: { detail: number; snapshot: number } }
      ).__orbitaFetchCounts;
      if (requestUrl.endsWith("/api/dashboard/v1/runs")) {
        counts.snapshot += 1;
      } else if (/\/api\/dashboard\/v1\/runs\/[^/]+$/u.test(requestUrl)) {
        counts.detail += 1;
      }
      if (requestUrl.endsWith("/api/dashboard/v1/runs") && response.ok) {
        (window as unknown as { __orbitaSnapshotResponseAt: number }).__orbitaSnapshotResponseAt =
          performance.now();
      }
      return response;
    };
      ...
      () => {
        new MutationObserver(() => {
          const state = window as unknown as {
            __orbitaExpectedLane: string;
            __orbitaExpectedTitle: string;
            __orbitaReconciliationLatency: number;
            __orbitaSnapshotResponseAt: number;
          };
          if (!state.__orbitaExpectedLane) {
            return;
          }
          const card = [...document.querySelectorAll<HTMLElement>(".run-card")].find((element) =>
            element.getAttribute("aria-label")?.startsWith(state.__orbitaExpectedTitle),
          );
          if (card?.closest<HTMLElement>(".lane")?.dataset.lane !== state.__orbitaExpectedLane) {
            return;
          }
          const validatedAt = performance
            .getEntriesByName("orbita-snapshot-validated")
            .at(-1)?.startTime;
          if (validatedAt == null) {
            return;
          }
    ...
    try {
      new PerformanceObserver((list) => {
        const durations = (window as unknown as { __orbitaLongTasks: Array<number> })
          .__orbitaLongTasks;
      ...
      // Long-task observation is optional in browsers without support.
    }
  });
}

async function emitInvalidations(
  page: Page,
  firstChangeId: number,
  count: number,
  reason: "snapshot_changed" | "observer_stale" | "observer_recovered" = "snapshot_changed",
) {
  await page.evaluate(
    async ({ count, firstChangeId, reason }) => {
      const source = (window as unknown as { __orbitaEventSources: Array<EventTarget> })
        .__orbitaEventSources[0];
...
test("responsive attention board, detail semantics, virtualization, and keyboard proof", async ({
  page,
}, testInfo) => {
  const snapshot = await mockDashboard(
    page,
    testInfo.project.name === "mobile" ? "waiting" : "balanced",
  );
  await page.goto("/");
  ...
  await expect(waitingRegion.getByLabel(`${waitingCount} runs`, { exact: true })).toHaveText(
    String(waitingCount),
  );
  ...
  expect(
    await page
      .locator('.lane[data-lane="degraded"] .status-reason')
      .first()
  ...
  expect(interactiveSamples[Math.ceil(interactiveSamples.length * 0.95) - 1]).toBeLessThan(2000);
  ...
  for (let change = 0; change < 5; change += 1) {
    ...
    snapshot.runs[0] = {
      ...
      reason: {
        ...
        value: laneId === "needs_help" ? "Decision missing" : "Approval needed",
      },
    };
    ...
    reconciliationSamples.push(
      await page.evaluate(
        () =>
          (window as unknown as { __orbitaReconciliationLatency: number })
            .__orbitaReconciliationLatency,
      ),
    );
  }
  reconciliationSamples.sort((left, right) => left - right);
  expect(reconciliationSamples[Math.ceil(reconciliationSamples.length * 0.95) - 1]).toBeLessThan(
    100,
  );

  await page.evaluate(() => {
    (window as unknown as { __orbitaLongTasks: Array<number> }).__orbitaLongTasks.length = 0;
  });
  await page.evaluate(() => {
    const counts = (
      window as unknown as { __orbitaFetchCounts: { detail: number; snapshot: number } }
  ...
  const fetchCounts = await page.evaluate(
    () =>
      (window as unknown as { __orbitaFetchCounts: { detail: number; snapshot: number } })
        .__orbitaFetchCounts,
  );
  ...
  const longestTask = await page.evaluate(() =>
    Math.max(0, ...(window as unknown as { __orbitaLongTasks: Array<number> }).__orbitaLongTasks),
  );
  ...
  await expect(page.locator(".run-card:focus")).toHaveAttribute(
    "aria-label",
...
test("failure states never masquerade as empty success", async ({ page }, testInfo) => {
  ...
  await page.route("**/api/dashboard/v1/events", (route) =>
    route.fulfill({ body: "", contentType: "text/event-stream", status: 200 }),
  );
  await page.route("**/api/dashboard/v1/runs", (route) =>
    route.fulfill({
      json: { error: { code: "observer_unavailable", message: "Observer unavailable" } },
      status: 503,
    }),
  );
  await page.goto("/");
  ...
  await page.route("**/api/dashboard/v1/runs", (route) =>
    route.fulfill({
      json: {
        error: { code: "invalid_request", message: "Dashboard runs root is not configured" },
      },
      status: 400,
    }),
  );
...
test("empty, stale, detail-error, and missing-selection states render explicitly", async ({
  page,
}, testInfo) => {
  test.skip(testInfo.project.name === "mobile");
  await installStableEvents(page);
  ...
  await page.route(/\/api\/dashboard\/v1\/runs\/run-proof-\d+$/, (route) =>
    route.fulfill({
      json: { error: { code: "observer_unavailable", message: "Unavailable" } },
      status: 503,
    }),
  );
  await page.locator(".run-card").first().click();
