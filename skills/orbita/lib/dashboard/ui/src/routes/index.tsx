...
export const Route = createFileRoute('/')({ validateSearch: searchSchema, component: DashboardRoute });
...
function DashboardRoute() {
  ...
  const events = useDashboardEvents(snapshot.data ? { changeId: snapshot.data.freshness.observerRevision, state: snapshot.data.freshness.state } : undefined, search.run);
  const orderedRuns = useStableLaneOrder(snapshot.data?.runs ?? [], events.reconciliation);
  const filters = useMemo(() => ({ q: search.q ?? '', workflow: search.workflow, lane: search.lane }), [search.q, search.workflow, search.lane]);
  ...
  const updateFilters = useCallback((change: Partial<typeof filters>) => void navigate({ search: (previous) => ({ ...previous, ...change }), replace: true }), [navigate]);
  const closeDetail = useCallback(() => void navigate({ search: (previous) => ({ ...previous, run: undefined }), replace: true }), [navigate]);
  const selectRun = useCallback((runId: string) => void navigate({ search: (previous) => ({ ...previous, run: runId }) }), [navigate]);
  const returnFocus = useCallback(() => {
    if (search.run) roving.focusRun(search.run, selectedLane);
  }, [roving, search.run, selectedLane]);
  if (snapshot.isPending) return <div className="dashboard-shell"><BoardLoading /></div>;
  if (!snapshot.data) {
    const retry = () => void snapshot.refetch();
    return snapshot.error instanceof DashboardFetchError && snapshot.error.code === 'invalid_request' ? <EmptyRoot onRetry={retry} /> : <SnapshotError onRetry={retry} />;
  }
  const freshness = selectFreshness(snapshot.data.freshness, { transport: events.transport, eventStale: events.observerStale, httpFailed: snapshot.isError, now });
  return <BoardScreen
