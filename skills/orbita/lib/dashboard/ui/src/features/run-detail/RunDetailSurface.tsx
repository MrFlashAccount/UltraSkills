...
export function RunDetailSurface(props: DetailSurfaceProps) {
  ...
  useEffect(() => { if (wide === true && props.selectedId) closeRef.current?.focus(); }, [wide, props.selectedId]);
  ...
  >{detailContent(props)}</Sheet>;
  const closeWide = () => { props.onClose(); requestAnimationFrame(props.onReturnFocus); };
  const body = detailContent({ ...props, onClose: closeWide });
  return <aside className="detail-panel" aria-label="Run details" onKeyDown={(event) => { if (event.key === 'Escape') closeWide(); }}>
    <header className="detail-header"><div><Badge>{props.detail ? LANE_LABELS[props.detail.laneId] : 'Run details'}</Badge><h2 className="detail-title">{props.detail?.title.value ?? 'Run details'}</h2></div><Button ref={closeRef} variant="quiet" size="icon" aria-label="Close details" onKeyDown={(event) => { if (event.key === 'Tab' && event.shiftKey) { event.preventDefault(); props.onReturnFocus(); } }} onClick={closeWide}><X aria-hidden="true" size={18} /></Button></header>
    {body}
  </aside>;
}

function detailContent({ visibleInResults, detail, isLoading, isError, onClose }: DetailSurfaceProps) {
