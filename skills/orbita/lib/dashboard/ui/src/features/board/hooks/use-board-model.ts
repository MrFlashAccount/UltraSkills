...
export function useBoardModel(runs: readonly RunSummaryDTO[], filters: BoardFilters) {
  ...
  return useMemo(() => {
    ...
    const lanes = Object.fromEntries(
      Object.entries(nextLanes).map(([lane, values]) => {
...
function sameRuns(previous: readonly RunSummaryDTO[], next: readonly RunSummaryDTO[]) {
  return (
    ...
    previous.every((run, index) => {
      const candidate = next[index];
      return (
        run.runId === candidate.runId &&
        run.title.value === candidate.title.value &&
        run.reason?.value === candidate.reason?.value &&
        run.workflow === candidate.workflow &&
        run.laneId === candidate.laneId &&
        run.status === candidate.status &&
        run.createdAt === candidate.createdAt &&
        run.updatedAt === candidate.updatedAt &&
        run.currentStep === candidate.currentStep &&
        run.cursor.kind === candidate.cursor.kind &&
        (run.cursor.kind !== 'single' ||
          (candidate.cursor.kind === 'single' && run.cursor.step === candidate.cursor.step)) &&
        run.occupancy.state === candidate.occupancy.state
      );
    })
  );
}
