...
export function RunCard({ run, selected, onSelect, roving, ensureVisible }: RunCardProps) {
  ...
  return <button
    ...
    onFocus={() => { roving.current.current = run.runId; }}
    onKeyDown={(event) => roving.onCardKeyDown(event, run.runId)}
    onClick={(event) => onSelect(run.runId, event.currentTarget)}
  >
    <span className="card-top"><span className="status-reason"><Icon aria-hidden="true" size={13} /><span>{unsupported ? 'Unsupported cursor' : run.reason?.value ?? run.status ?? 'Status update'}</span></span><time dateTime={run.updatedAt}>{formatAge(run.updatedAt ?? run.createdAt)}</time></span>
    <strong className="card-title">{run.title.value}</strong>
    <span className="card-fact"><span>Workflow</span><b>{run.workflow}</b></span>
    <span className="card-fact"><span>Current step</span><code>{unsupported ? 'unsupported' : run.currentStep ?? 'None'}</code></span>
    {!run.title.value.includes(run.runId) ? <code className="card-id">{shortRunId(run.runId)}</code> : null}
  </button>;
}
