...
export const Lane = memo(function Lane({ lane, runs, selectedId, onSelect, roving }: LaneProps) {
  ...
  return <CollapsibleRoot open={disclosure.expanded} onOpenChange={disclosure.setExpanded} className="lane" data-lane={lane} role="region" aria-labelledby={headingId}>
    <header className="lane-header">
      <h2 ref={(element) => roving.registerLaneHeader(lane, element)} tabIndex={-1} id={headingId} className="lane-title">{LANE_LABELS[lane]}</h2><span className="lane-count" aria-label={`${runs.length} runs`}>{runs.length}</span>
      <CollapsibleTrigger className="lane-toggle" aria-label={`${disclosure.expanded ? 'Collapse' : 'Expand'} ${LANE_LABELS[lane]}`} aria-controls={listId}>
        {disclosure.expanded ? <ChevronUp aria-hidden="true" size={17} /> : <ChevronDown aria-hidden="true" size={17} />}
      </CollapsibleTrigger>
    </header>
    <CollapsibleContent id={listId} className="lane-body"><VirtualLane laneLabel={LANE_LABELS[lane]} runs={runs} selectedId={selectedId} onSelect={onSelect} roving={roving} /></CollapsibleContent>
  </CollapsibleRoot>;
});
