...
import { DashboardReadModel, ObserverUnavailableError } from '../../../../dashboard/observer/dashboard-read-model.server';
...
function boundedInteger(name: string, value: string | undefined, fallback: number, min: number, max: number): number {
  ...
  if (!Number.isSafeInteger(parsed) || parsed < min || parsed > max) throw new Error(`${name} is outside the allowed range`);
  return parsed;
}

export function dashboardServerConfig(env: Record<string, string | undefined> = process.env): DashboardServerConfig {
  ...
  if (!statSync(runsRoot, { throwIfNoEntry: false })?.isDirectory()) throw new DashboardConfigurationError('Dashboard runs root is not configured');
  const host = env.ORBITA_DASHBOARD_HOST ?? env.NITRO_HOST ?? env.HOST ?? '127.0.0.1';
  if (!/^[A-Za-z0-9.:[\]-]{1,255}$/u.test(host)) throw new Error('ORBITA_DASHBOARD_HOST is invalid');
  return {
    runsRoot,
    host,
    port: boundedInteger('ORBITA_DASHBOARD_PORT', env.ORBITA_DASHBOARD_PORT ?? env.NITRO_PORT ?? env.PORT, 3000, 0, 65_535),
    pollMs: boundedInteger('ORBITA_DASHBOARD_POLL_MS', env.ORBITA_DASHBOARD_POLL_MS, 2_000, 250, 300_000),
    heartbeatMs: boundedInteger('ORBITA_DASHBOARD_HEARTBEAT_MS', env.ORBITA_DASHBOARD_HEARTBEAT_MS, 15_000, 1_000, 120_000),
    staleMs: boundedInteger('ORBITA_DASHBOARD_STALE_MS', env.ORBITA_DASHBOARD_STALE_MS, 10_000, 1_000, 600_000),
    coalesceMs: boundedInteger('ORBITA_DASHBOARD_COALESCE_MS', env.ORBITA_DASHBOARD_COALESCE_MS, 100, 10, 1_000),
  };
}

export function createDashboardComposition(
  config = dashboardServerConfig(),
  reader: Pick<RunsRootObserverReader, 'listRuns' | 'getRun'> = new RunsRootObserverReader(config.runsRoot),
) {
  ...
  readModel.start();
  return { config, readModel, close: () => readModel.close() };
}
...
export function bindDashboardLifecycle(
  ...
  const close = async () => { await closeOperation(); };
  const onBeforeExit = () => { void close(); };
  const onSignal = () => { void close().finally(() => terminate(0)); };
